...
Status: modular Tauri runtime provisioning and narrow workflow commands complete; interactive UI-driven and live SRT-backed lifecycle validation are next
...
- Desktop runtime construction is implemented behind replaceable workspace, secret, port, versioned tool-bundle/asset, process-launch, and readiness adapters. It starts separate supervised broker/opencode child trees, caps and drains child logs, attests exact listener ownership, clears and isolates the opencode environment, validates the provider implementation/base URL/exact model registration, selects a dedicated default-deny agent, transfers broker bootstrap secrets over inherited stdin, constructs independent model/authorization handles, and rolls back partial startup in reverse order. A transport-neutral single-job application runner now exposes that controller through explicit asynchronous Tauri commands and normalized events. Rust owns native DOCX selection and returns only a single-use opaque ID; a replaceable path policy rejects non-local Windows and reparse-point selections. Cooperative startup cancellation, app-owned cleanup roots, target-aware relocatable runtime assets, and executable/integrity-verified local prerequisite health are enforced behind replaceable boundaries. Automated evidence is in `docs/test-plans/spike-001-tauri-runtime-composition.md`; interactive invocation through the minimal UI remains pending.
...
- Interactive UI-driven validation of approval, rejection, cancellation, crash recovery, and app shutdown for the composed pinned opencode and broker-host process trees on Windows. The live provisioner now passes real broker/opencode startup, challenge-proven and process-tree-attested readiness, effective-profile validation, session/event-stream connection, listener/process cleanup, and transient-workspace removal. A narrow asynchronous Tauri command/event boundary now composes that provisioner, and controller shutdown exhaustively cleans all active jobs; the remaining gate is to drive it through the minimal UI and perform the live SRT-backed revised-copy workflow.
