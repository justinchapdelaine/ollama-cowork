...
Status: modular Tauri runtime, narrow workflow commands, and interactive allow-once SRT-backed DOCX publication complete; reject/cancel/crash lifecycle validation remains
Scope: one DOCX workflow using Tauri, opencode, remote Ollama, SRT, and clean-room DOCX tooling
Verification date: 2026-07-15 (America/Vancouver)
...
| Hidden opencode backend | Verified through desktop composition and allow-once publication | The proof-recorded opencode `1.17.18` build ran on authenticated localhost with restrictive configuration, remote model-metadata fetching disabled, `bash` hidden from the selected agent, exact custom-tool inventory, real model prompt, trusted read-only inspection, an informed structured rewrite proposal, and SRT-backed revised-copy publication. |
| Pre-installed opencode | Verified spike prerequisite with exact identity | Version alone was insufficient because two Windows builds reported `1.17.18`; desktop health and the runtime launch boundary now pin the passing proof executable length and SHA-256 and fail closed on a different build. Packaging or installing it remains out of scope. |
...
The Rust host owns approval state. An opencode permission request is necessary but not sufficient: the host parses the exact structured rewrite proposal, maps it to a known action, displays a friendly summary, binds the one-time decision to that operation, and only then responds. OpenCode may label the custom tool `running` while its own `context.ask` is still unresolved, so adapter lifecycle states do not define trusted execution. The controller starts the trusted mutation phase only after both the exact one-time broker authorization and the matching permission reply succeed. The broker rejects changed arguments without consuming approval. Do not expose opencode's session-level “always” choice in Spike 001.
...
SRT is an Apache-2.0 beta research preview. Windows support is documented as **alpha**, with bundled x64/arm64 `srt-win.exe` helpers and a one-time elevated installation. On 2026-07-15, the official npm package reported `0.0.65` as the newest published version. The package version and bundled helper identity are distinct: the proof-tested `0.0.65` Windows helper reports `srt-win 0.0.1`. Resolve the newest published package immediately before each upgrade evaluation:
...
Do not silently install that result at application startup. Treat it as an update candidate: install it in an isolated validation location, record its package version, helper-reported version, architecture, file length, SHA-256, and upstream integrity, then rerun the complete Windows SRT enforcement matrix. Advance the application pin only after the candidate passes. This keeps update discovery frequent while production workflows continue using the last proof-tested identity.
...
- Desktop runtime construction is implemented behind replaceable workspace, secret, port, versioned tool-bundle/asset, deny-only prompt-profile, process-launch, readiness, executable-identity, trusted-output-decoder, and model-event adapters. It starts separate supervised broker/opencode child trees, caps and drains child logs, attests exact listener ownership, clears and isolates the opencode environment, validates the provider/base URL/model and exact custom-tool ID/description/schema mapping, explicitly selects a dedicated default-deny agent for every request, states that one attachment is already job-bound without exposing its path, and separates job/source-correlated read-only inspection from approval-gated mutation events. The proof-recorded opencode executable is pinned by version, length, and SHA-256 and is re-attested immediately before each launch attempt; bounded version/readiness retries do not relax configuration, identity, or policy validation. Model-controlled identifiers, text, messages, tool calls, inspection sections, and event delivery are bounded. The minimal frontend composes an injectable Tauri transport, pure workflow state machine, scheduler-driven polling coordinator, and text-only DOM renderer; terminal events survive in-flight polling failures, disposal cannot restart in-flight polling, partial listener setup is rolled back, and streamed assistant deltas form one bounded message. The live composed runtime reaches an exact rewrite approval request in two fresh `gemma4:12b` sessions, and the real Tauri UI now completes allow-once publication through SRT and the clean-room DOCX tool. Independent validation matched the UI's revised digest, preserved the source digest and adjacent canary, and reopened the published package successfully. Reject/cancel/crash lifecycle paths remain pending.
...
- Interactive UI-driven validation of rejection, cancellation, crash recovery, and app shutdown for the composed pinned opencode and broker-host process trees on Windows. The live provisioner passes real broker/opencode startup, challenge-proven and process-tree-attested readiness, effective-profile validation, session/event-stream connection, listener/process cleanup, and transient-workspace removal. The narrow asynchronous Tauri command/event boundary now passes from synthetic file selection through allow-once SRT-backed revised-copy publication; the remaining gate is to exercise the other decisions and failure lifecycles.
...
No remaining question blocks continuing the live integrated validation. The following items must be resolved or validated before calling the desktop prototype complete:

1. **Tauri configuration isolation:** The live provisioner now verifies the isolated profile, restrictive global/dedicated-agent permissions, disabled sharing/autoupdate and remote model-metadata fetching, request-scoped deny-only `bash` visibility, explicit agent/context serialization, withheld broker control/job credentials, exact executable identity at launch, exact custom-tool ID/description/schema mappings, and job/source-correlated inspection output before creating or trusting a session result. Retain these checks as regression gates while completing UI-driven approval and publication.
